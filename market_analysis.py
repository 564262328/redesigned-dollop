...
def run():
    ...
    sentiment_score = int((up_count / len(df_a)) * 100) if len(df_a) > 0 else 50
            ...
            <div class="glass-card p-5 border-t-2 border-white/5 hover:border-blue-500/50 transition-all font-black uppercase">
                <div class="flex justify-between items-start mb-4 font-black">
                    ...
                    <div class="text-right">
                        <span class="text-xl font-black {color}">¥{price}</span>
                        <span class="text-xs {color} block font-bold tracking-tighter">{pct}%</span>
                    </div>
                </div>
    ...
    new_stocks_display = "、".join(new_stocks_list) if new_stocks_list else "No New Listings Found Today"
...
<body class="p-4 md:p-12 font-bold italic font-black uppercase tracking-tight">
    <div class="max-w-6xl mx-auto font-bold italic">
        <header class="flex flex-col md:flex-row justify-between items-center mb-12 border-b border-white/5 pb-10 gap-6">
            <div>
                <h1 class="text-4xl font-black italic tracking-tighter bg-clip-text text-transparent bg-gradient-to-br from-blue-400 to-emerald-400 uppercase">QUANT TERMINAL v13.2</h1>
            ...
            <div class="bg-slate-900/80 px-5 py-3 rounded-2xl border border-white/10 shadow-2xl">
                <span class="text-blue-400 font-black text-[10px] block mb-1 tracking-widest animate-pulse italic font-black uppercase tracking-widest">● SYNC ACTIVE</span>
                <span class="text-sm font-mono text-slate-300 tracking-widest italic font-bold uppercase">{now_str}</span>
            </div>
        </header>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16 font-bold">
            {cards_html}
        </div>

        <!-- Monitoring Section -->
        <div class="glass-card p-8 border border-blue-500/20 font-bold">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6 font-bold">
                <h2 class="text-xs font-black text-blue-400 uppercase tracking-[0.3em] italic font-bold">DATA ASSETS MONITOR / 數據資產</h2>
                <div class="flex gap-8">
                    <div class="text-center font-bold">
                        <span class="block text-2xl font-black text-white font-mono font-bold italic">{db['total_count']}</span>
            ...
            <div class="bg-black/40 rounded-xl p-5 border border-white/5 font-bold italic font-black uppercase">
                <p class="text-[11px] text-slate-400 tracking-widest leading-relaxed"><span class="text-blue-500 font-bold">▶</span> New Listings Log: {new_stocks_display}</p>
            </div>
        </div>

        <footer class="mt-20 py-10 text-center border-t border-white/5 text-slate-700 text-[10px] uppercase tracking-[0.4em] font-bold">
            Persistence Engine: Git-Auto-Commit • v13.2 Stable • 2026-03-18
        </footer>
    </div>
</body>
</html>
"""
    # Final Output Files (Lowercase index.html is mandatory)
