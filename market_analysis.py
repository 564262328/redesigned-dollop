...
# --- 1. Database & Helper Functions (Must stay at the top) ---
...
def run():
    ...
    # Fetch Spot Data & Industry Rankings
    df_a = fetch_safe_data(ak.stock_zh_a_spot_em)
    df_sector = fetch_safe_data(ak.stock_board_industry_name_em) # Industry Sector Ranking
    ...
    # Process Individual Stock Cards (Top 12 Gainer)
    ...
    <div class="glass-card p-5 border-t-2 border-white/5 hover:border-blue-500/50 transition-all font-black uppercase">
        <div class="flex justify-between items-start mb-4">
            ...
            <div class="text-right"><span class="text-xl font-black text-red-500">¥{r['最新价']}</span><span class="text-xs text-red-500 block font-bold">{r['涨跌幅']}%</span></div>
        </div>
        <div class="grid grid-cols-2 gap-3 text-[10px] border-t border-white/5 pt-3 uppercase italic font-bold">
            <div class="flex flex-col"><span class="text-slate-600">换手率</span><span class="text-slate-200">{r['换手率']}%</span></div>
            <div class="flex flex-col text-right"><span class="text-slate-600">量比</span><span class="text-amber-400">{r['量比']}</span></div>
        </div>
...
<body class="p-4 md:p-12 font-black uppercase tracking-tight italic">
    <div class="max-w-6xl mx-auto">
        ...
        <!-- TOP 5 INDUSTRY SECTORS (NEW MODULE) -->
        <div class="mb-12 glass-card p-8 border border-red-500/20 bg-red-500/5">
            <h2 class="text-xs font-black text-red-500 uppercase tracking-[0.3em] mb-6 flex items-center gap-2">
                <span class="w-2 h-2 bg-red-500 rounded-full animate-ping"></span> 領漲行業板塊 / TOP 5 INDUSTRY SECTORS
            </h2>
            <div class="grid grid-cols-1 md:grid-cols-5 gap-8">
                {sector_html if sector_html else "<p class='text-slate-600 italic'>Loading Market Breadth...</p>"}
            </div>
        </div>
        ...
        <div class="glass-card p-8 border border-blue-500/20">
            <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-6 font-bold">
                ...
                <div class="flex gap-8">
                    <div class="text-center font-bold font-black"><span class="block text-2xl font-black text-white font-mono font-bold font-black">{db['total_count']}</span><span class="text-[9px] text-slate-500 uppercase">Total Stocks</span></div>
                    <div class="text-center border-l border-white/10 pl-8 font-bold font-black"><span class="block text-2xl font-black text-emerald-400 font-mono font-bold font-black">+{len(new_stocks)}</span><span class="text-[9px] text-slate-500 uppercase font-black">New Today</span></div>
                </div>
            </div>
...
"""
    ...
    print(f"🚀 v13.4 Market Leadership Terminal Deployed: {now_str}")
