...
def get_full_code(name_or_code):
    try:
        ...
        if not match.empty:
            ...
            return full_code, match.iloc[0]['名称']
...
def run():
    ...
    content = content.replace('⚠️', '<span class="px-2 py-0.5 bg-amber-900/50 text-amber-400 rounded text-xs border border-amber-700/50">警告 ⚠️</span>')
...
<body class="p-4 md:p-10 text-sm md:text-base">
