...
if __name__ == "__main__":
    ...
    if not market_df.empty:
        codes = [str(c) for c in market_df['代码'].tolist()]
        old_codes = set(current_db.get("stock_list", []))
        if old_codes:
            diff_count = len(set(codes) - old_codes)
        current_db.update({"last_update": now_str, "total_count": len(codes), "stock_list": codes})
        save_db(current_db)
    build_html(now_str, current_db, market_df, diff_count)
